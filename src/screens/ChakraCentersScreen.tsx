...
const getChakraCentersAudio = () => require('../../assets/audio/the-chakra-centers.mp3');

export const ChakraCentersScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(CHAKRA_CENTERS_DURATION_SEC * 1000);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getChakraCentersAudio(), {
        ...
        onError: (error) => {
          console.error('Chakra Centers playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Chakra Centers audio:', error);
      }
      setIsLoading(false);
    }
    setCountdown(5);
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, CHAKRA_CENTERS_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_chakra_centers_${Date.now()}`,
      date: today,
      templateId: 'extra_chakra_centers',
    ...
    navigation.navigate('SessionComplete', {
      sessionTitle: 'The Chakra Centers',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest attention in each center.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The Chakra Centers</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A direct investigation into the body's seven centers of sensation.
        </Text>
