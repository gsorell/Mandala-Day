...
const getChildrensSleepAudio = () => require('../../assets/audio/body-safari.mp3');

export const ChildrensSleepScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(CHILDRENS_SLEEP_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, CHILDRENS_SLEEP_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getChildrensSleepAudio(), {
        ...
        onError: (error) => {
          console.error('Children\'s Sleep playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Children\'s Sleep audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    navigation.navigate('SessionComplete', {
      ...
      shareMessage: 'A bedtime journey for the little ones',
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Jungle Safari</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle, imagination-filled bedtime meditation for children.
        </Text>
