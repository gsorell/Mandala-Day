...
const THE_CITY_OF_LIGHTS_DURATION_MIN = 8; // ~7:42
const THE_CITY_OF_LIGHTS_DURATION_SEC = THE_CITY_OF_LIGHTS_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getTheCityOfLightsAudio = () => require('../../assets/audio/the-city-of-lights.mp3');

export const CityOfLightsScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(THE_CITY_OF_LIGHTS_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, THE_CITY_OF_LIGHTS_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getTheCityOfLightsAudio(), {
        ...
        onError: (error) => {
          console.error('The City of Lights playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load The City of Lights audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, THE_CITY_OF_LIGHTS_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_city_of_lights_${Date.now()}`,
      date: today,
      templateId: 'extra_city_of_lights',
    ...
    navigation.navigate('SessionComplete', {
      sessionTitle: 'The City of Lights',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest at the window. Watch the city go dark.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The City of Lights</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A quiet, city-at-night bedtime meditation for children.
        </Text>
