...
const CROWN_TO_SOLE_DURATION_MIN = 10;
const CROWN_TO_SOLE_DURATION_SEC = CROWN_TO_SOLE_DURATION_MIN * 60;

const getCrownToSoleAudio = () => require('../../assets/audio/crown-to-sole.mp3');

export const CrownToSoleScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(CROWN_TO_SOLE_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, CROWN_TO_SOLE_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getCrownToSoleAudio(), {
        ...
        onError: (error) => {
          console.error('Crown to Sole playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Crown to Sole audio:', error);
      }
      setIsLoading(false);
    }
    setCountdown(5);
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, CROWN_TO_SOLE_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_crown_to_sole_${Date.now()}`,
      date: today,
      templateId: 'extra_crown_to_sole',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Down through the body, crown to sole.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Crown to Sole</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A slow descent of attention through the body, crown to sole.
        </Text>

        <Text style={styles.instruction}>
          Sit or lie down, and begin when ready.
        </Text>
      </View>
