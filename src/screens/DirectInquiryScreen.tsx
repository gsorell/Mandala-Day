...
const DIRECT_INQUIRY_DURATION_MIN = 10;
const DIRECT_INQUIRY_DURATION_SEC = DIRECT_INQUIRY_DURATION_MIN * 60;

const getDirectInquiryAudio = () => require('../../assets/audio/direct-inquiry.mp3');

export const DirectInquiryScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(DIRECT_INQUIRY_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, DIRECT_INQUIRY_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getDirectInquiryAudio(), {
        ...
        onError: (error) => {
          console.error('Direct Inquiry playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Direct Inquiry audio:', error);
      }
      setIsLoading(false);
    }
    setCountdown(5);
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, DIRECT_INQUIRY_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_direct_inquiry_${Date.now()}`,
      date: today,
      templateId: 'extra_direct_inquiry',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Turn awareness on itself.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Direct Inquiry</Text>
