...
const THE_FIREFLY_MEADOW_DURATION_MIN = 7; // ~7:00
const THE_FIREFLY_MEADOW_DURATION_SEC = THE_FIREFLY_MEADOW_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getTheFireflyMeadowAudio = () => require('../../assets/audio/the-firefly-meadow.mp3');

export const FireflyMeadowScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(THE_FIREFLY_MEADOW_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, THE_FIREFLY_MEADOW_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getTheFireflyMeadowAudio(), {
        ...
        onError: (error) => {
          console.error('The Firefly Meadow playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load The Firefly Meadow audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, THE_FIREFLY_MEADOW_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_firefly_meadow_${Date.now()}`,
      date: today,
      templateId: 'extra_firefly_meadow',
    ...
    navigation.navigate('SessionComplete', {
      sessionTitle: 'The Firefly Meadow',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest in the grass. Watch the little lights.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The Firefly Meadow</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A quiet, firefly-lit bedtime meditation for children.
        </Text>
