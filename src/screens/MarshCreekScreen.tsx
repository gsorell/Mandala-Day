...
const MARSH_CREEK_DURATION_MIN = 7; // ~6:16 rounded up
const MARSH_CREEK_DURATION_SEC = MARSH_CREEK_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getMarshCreekAudio = () => require('../../assets/audio/marsh-creek.mp3');

export const MarshCreekScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(MARSH_CREEK_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, MARSH_CREEK_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getMarshCreekAudio(), {
        title: 'Marsh Creek',
        artist: 'Mandala Day',
        ...
        onError: (error) => {
          console.error('Marsh Creek playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getMarshCreekAudio(), {
          title: 'Marsh Creek',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load Marsh Creek audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, MARSH_CREEK_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_marsh_creek_${Date.now()}`,
      date: today,
      templateId: 'extra_marsh_creek',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Marsh Creek</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle streamside meditation for children.
        </Text>
