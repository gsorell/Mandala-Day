...
const THE_PLAY_FORT_DURATION_MIN = 7; // ~6:47 rounded up
const THE_PLAY_FORT_DURATION_SEC = THE_PLAY_FORT_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getThePlayFortAudio = () => require('../../assets/audio/the-play-fort.mp3');

export const PlayFortScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(THE_PLAY_FORT_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, THE_PLAY_FORT_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getThePlayFortAudio(), {
        title: 'The Play Fort',
        artist: 'Mandala Day',
        ...
        onError: (error) => {
          console.error('The Play Fort playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getThePlayFortAudio(), {
          title: 'The Play Fort',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load The Play Fort audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, THE_PLAY_FORT_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_play_fort_${Date.now()}`,
      date: today,
      templateId: 'extra_play_fort',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Settle into the little fort. Drift off.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The Play Fort</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A cozy, lantern-lit bedtime meditation for children.
        </Text>
