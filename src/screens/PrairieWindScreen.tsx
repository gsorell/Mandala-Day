...
const PRAIRIE_WIND_DURATION_MIN = 9; // ~8:58 rounded up
const PRAIRIE_WIND_DURATION_SEC = PRAIRIE_WIND_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getPrairieWindAudio = () => require('../../assets/audio/prairie-wind.mp3');

export const PrairieWindScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(PRAIRIE_WIND_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, PRAIRIE_WIND_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getPrairieWindAudio(), {
        title: 'Prairie Wind',
        artist: 'Mandala Day',
        ...
        onError: (error) => {
          console.error('Prairie Wind playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getPrairieWindAudio(), {
          title: 'Prairie Wind',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load Prairie Wind audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, PRAIRIE_WIND_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_prairie_wind_${Date.now()}`,
      date: today,
      templateId: 'extra_prairie_wind',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Relax. Breathe. Drift off.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Prairie Wind</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle, windswept meditation for children.
        </Text>
