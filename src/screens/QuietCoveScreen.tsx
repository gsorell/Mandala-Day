...
const QUIET_COVE_DURATION_MIN = 7; // ~6:51 rounded up
const QUIET_COVE_DURATION_SEC = QUIET_COVE_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getQuietCoveAudio = () => require('../../assets/audio/quiet-cove.mp3');

export const QuietCoveScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(QUIET_COVE_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, QUIET_COVE_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getQuietCoveAudio(), {
        ...
        onError: (error) => {
          console.error('Quiet Cove playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Quiet Cove audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, QUIET_COVE_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_quiet_cove_${Date.now()}`,
      date: today,
      templateId: 'extra_quiet_cove',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The Quiet Cove</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A calm, seaside meditation for children.
        </Text>
