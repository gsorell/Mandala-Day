...
const RAIN_ON_THE_ROOF_DURATION_MIN = 8; // ~7:44
const RAIN_ON_THE_ROOF_DURATION_SEC = RAIN_ON_THE_ROOF_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getRainOnTheRoofAudio = () => require('../../assets/audio/rain-on-the-roof.mp3');

export const RainOnTheRoofScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(RAIN_ON_THE_ROOF_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, RAIN_ON_THE_ROOF_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getRainOnTheRoofAudio(), {
        ...
        onError: (error) => {
          console.error('Rain on the Roof playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getRainOnTheRoofAudio(), {
          title: 'Rain on the Roof',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load Rain on the Roof audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, RAIN_ON_THE_ROOF_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_rain_on_the_roof_${Date.now()}`,
      date: today,
      templateId: 'extra_rain_on_the_roof',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest by the window. Listen to the rain.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Rain on the Roof</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A quiet, rainy-night bedtime meditation for children.
        </Text>
