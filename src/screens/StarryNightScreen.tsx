...
const getStarryNightAudio = () => require('../../assets/audio/starry-night.mp3');

export const StarryNightScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(STARRY_NIGHT_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, STARRY_NIGHT_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getStarryNightAudio(), {
        ...
        onError: (error) => {
          console.error('Starry Night playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load Starry Night audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    navigation.navigate('SessionComplete', {
      ...
      shareMessage: 'A starry night journey for the little ones',
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Starry Night</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle, starlit bedtime meditation for children.
        </Text>
