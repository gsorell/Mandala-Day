...
const THE_FIRST_SNOW_DURATION_MIN = 10; // ~9:09 rounded up
const THE_FIRST_SNOW_DURATION_SEC = THE_FIRST_SNOW_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getTheFirstSnowAudio = () => require('../../assets/audio/the-first-snow.mp3');

export const TheFirstSnowScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(THE_FIRST_SNOW_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, THE_FIRST_SNOW_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getTheFirstSnowAudio(), {
        ...
        onError: (error) => {
          console.error('The First Snow playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      try {
        ...
        console.error('Failed to pre-load The First Snow audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, THE_FIRST_SNOW_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_the_first_snow_${Date.now()}`,
      date: today,
      templateId: 'extra_the_first_snow',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The First Snow</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle, snow-hushed bedtime meditation for children.
        </Text>
