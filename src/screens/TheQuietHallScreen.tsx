...
const QUIET_HALL_DURATION_MIN = 7; // ~7:05
const QUIET_HALL_DURATION_SEC = QUIET_HALL_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getQuietHallAudio = () => require('../../assets/audio/the-quiet-hall.mp3');

export const TheQuietHallScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(QUIET_HALL_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, QUIET_HALL_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getQuietHallAudio(), {
        ...
        onError: (error) => {
          console.error('The Quiet Hall playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getQuietHallAudio(), {
          title: 'The Quiet Hall',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load The Quiet Hall audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, QUIET_HALL_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_the_quiet_hall_${Date.now()}`,
      date: today,
      templateId: 'extra_the_quiet_hall',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest in the long room. Watch the small flame.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>The Quiet Hall</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A quiet, candlelit bedtime meditation for children.
        </Text>
