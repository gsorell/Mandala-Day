...
const PARK_SLEEPS_DURATION_MIN = 7; // ~7:15
const PARK_SLEEPS_DURATION_SEC = PARK_SLEEPS_DURATION_MIN * 60;

// Audio asset - using require for bundling
const getParkSleepsAudio = () => require('../../assets/audio/when-the-park-sleeps.mp3');

export const WhenTheParkSleepsScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(PARK_SLEEPS_DURATION_SEC * 1000);
  ...
  usePracticeNotificationGuard(isPlaying, PARK_SLEEPS_DURATION_MIN);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getParkSleepsAudio(), {
        ...
        onError: (error) => {
          console.error('When the Park Sleeps playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getParkSleepsAudio(), {
          title: 'When the Park Sleeps',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load When the Park Sleeps audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, PARK_SLEEPS_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_when_the_park_sleeps_${Date.now()}`,
      date: today,
      templateId: 'extra_when_the_park_sleeps',
    ...
    navigation.navigate('SessionComplete', {
      sessionTitle: 'When the Park Sleeps',
  ...
  if (isPlaying) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.meditationView}>
          <Text style={styles.timerLarge}>{formatTime(duration - currentPosition)}</Text>
          <Text style={styles.meditationPrompt}>Rest by the carousel. Watch the lights go down.</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>When the Park Sleeps</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A quiet, lantern-lit bedtime meditation for children.
        </Text>
