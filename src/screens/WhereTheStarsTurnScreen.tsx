...
const getWhereTheStarsTurnAudio = () => require('../../assets/audio/where-the-stars-turn.mp3');

export const WhereTheStarsTurnScreen: React.FC = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [isPlaying, setIsPlaying] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [currentPosition, setCurrentPosition] = useState(0);
  const [duration, setDuration] = useState(WHERE_THE_STARS_TURN_DURATION_SEC * 1000);
  ...
  const playAudio = async () => {
    ...
    if (Platform.OS === 'web' && audioService.isLoaded()) {
      ...
      await audioService.loadAndPlay(getWhereTheStarsTurnAudio(), {
        ...
        onError: (error) => {
          console.error('Where the Stars Turn playback error:', error);
          setIsPlaying(false);
        },
      });
    }
  ...
  const handleStart = async () => {
    if (Platform.OS === 'web') {
      ...
      setIsLoading(true);
      try {
        await audioService.preload(getWhereTheStarsTurnAudio(), {
          title: 'Where the Stars Turn',
          artist: 'Mandala Day',
          onRemoteStateChange: (state) => setIsPaused(state.isPaused),
          onPlaybackStatusUpdate: handlePlaybackStatus,
        ...
        console.error('Failed to pre-load Where the Stars Turn audio:', error);
      }
      setIsLoading(false);
    }
  ...
  const handleComplete = async () => {
    ...
    await addExtraPracticeMinutes(today, WHERE_THE_STARS_TURN_DURATION_MIN);
    await appendExtraInstance({
      id: `${today}_extra_where_the_stars_turn_${Date.now()}`,
      date: today,
      templateId: 'extra_where_the_stars_turn',
    ...
    navigation.navigate('SessionComplete', {
      sessionTitle: 'Where the Stars Turn',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>Where the Stars Turn</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.description}>
          A gentle bedtime journey through the turning night sky, for children.
        </Text>
